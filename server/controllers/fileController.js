...
fileController.searchStations = (req, res, next) => {
  try {
    ...
    const results = res.locals.stations.filter((station) => JSON.stringify(station['Stop Name']).toLowerCase().includes(searchId.toLowerCase()) && station['GTFS Stop ID'] !== 901 && station['GTFS Stop ID'] !== 902);
    results.forEach((result) => {
      ...
      for (const [key, value] of Object.entries(res.locals.routes)) {
        const routes = JSON.stringify(value.join(''));
